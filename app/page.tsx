...
import { 
  Trash2, CheckCircle, MessageSquare, Plus, Image as ImageIcon, Loader2, 
  UploadCloud, X, AlertTriangle, Pencil, CornerDownRight, ChevronDown, 
  ChevronUp, Film, FileText, FileImage, Download, Paperclip, Search, 
  Filter, TrendingUp 
} from 'lucide-react';
...
const firebaseConfig = {
  // ⚠️ GANTI DENGAN DATA DARI FIREBASE CONSOLE ANDA
...
export default function InspirationBoard() {
  ...
  // --- STATE LAINNYA (DELETE, KOMENTAR, USERNAME) ---
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(inspirationsRef, (snapshot) => {
      ...
      if (data) {
        const parsedItems: InspirationItem[] = Object.keys(data).map((key) => {
          // Handle legacy data / struktur attachment
    ...
    // Load username dari localStorage jika ada
  ...
  // ----------------------------------------------------------------
  // 3. FUNGSI UPLOAD (CLOUDINARY)
  // ----------------------------------------------------------------
  const uploadToCloudinary = async (file: File): Promise<string | null> => {
    const formData = new FormData();
    formData.append('file', file);
    formData.append('upload_preset', 'ml_default'); // Pastikan preset ini ada di Cloudinary Anda
    
    try {
      // Menggunakan endpoint 'auto' agar Cloudinary mendeteksi tipe file (image/video/raw)
  ...
  // ----------------------------------------------------------------
  // 4. LOGIC CRUD (CREATE, UPDATE, DELETE)
  // ----------------------------------------------------------------
  ...
  const addReplyToComment = (comments: Comment[], targetId: string, newReply: Comment): Comment[] => {
    return comments.map(c => 
      c.id === targetId 
        ...
        : { ...c, replies: c.replies ? addReplyToComment(c.replies, targetId, newReply) : [] }
    );
  };
  ...
  const editCommentRecursive = (comments: Comment[], targetId: string, newText: string): Comment[] => {
    return comments.map(c => 
      c.id === targetId 
        ? { ...c, text: newText } 
        : { ...c, replies: c.replies ? editCommentRecursive(c.replies, targetId, newText) : [] }
    );
  };
  ...
  const handleSendComment = (item: InspirationItem, parentCommentId: string | null = null) => {
    ...
    const updated = parentCommentId 
      ? addReplyToComment(item.comments, parentCommentId, newComment) 
  ...
  const handleDeleteComment = (item: InspirationItem, cid: string) => { 
    if (confirm('Hapus komentar ini?')) {
      updateCommentsInFirebase(item.id, deleteCommentRecursive(item.comments, cid)); 
    }
  };

  const handleEditComment = (item: InspirationItem, cid: string, txt: string) => {
    updateCommentsInFirebase(item.id, editCommentRecursive(item.comments, cid, txt));
  };
  ...
  // ----------------------------------------------------------------
  // 7. UI COMPONENT: MEDIA GALLERY & COMMENT
  // ----------------------------------------------------------------
      ...
      <div className="relative w-full h-72 bg-gray-900 group">
        <div className="flex overflow-x-auto snap-x snap-mandatory w-full h-full scrollbar-hide">
          {attachments.map((att, index) => (
            <div key={index} className="w-full flex-shrink-0 snap-center h-full relative flex items-center justify-center bg-gray-100">
              {att.type === 'video' ? (
                <video src={att.url} controls className="h-full max-w-full object-contain bg-black" />
              ) : att.type === 'file' ? (
                <div className="flex flex-col items-center justify-center p-6 text-center h-full w-full bg-slate-100 text-slate-600">
                  ...
                  <a href={att.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-[#B91C1C] text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-red-800 transition-colors">
        ...
        {attachments.length > 1 && (
          <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-1 z-10 pointer-events-none">
            {attachments.map((_, i) => <div key={i} className="w-1.5 h-1.5 rounded-full bg-white/50 shadow-sm" />)}
          </div>
        )}
      </div>
    );
  ...
  const CommentItem = ({ comment, item, depth = 0 }: { comment: Comment, item: InspirationItem, depth?: number }) => {
    ...
    const dateDisplay = (() => { 
      try { if (comment.createdAt.includes('T')) return new Date(comment.createdAt).toLocaleDateString('id-ID'); return comment.createdAt; } 
    ...
    return (
      <div className={cls}>
        <div className={`p-3 rounded-xl transition-colors group relative ${depth >= 3 ? 'bg-[#FFF5F5]' : 'bg-white border border-gray-100'}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              {!isEditingC && (
                <div className="flex gap-1">
                  <button onClick={() => setIsEditingC(true)} className="p-1 hover:bg-gray-100 rounded text-gray-400 hover:text-blue-500"><Pencil size={12} /></button>
          ...
          {isEditingC ? (
            <div className="mt-2">
              <textarea className="w-full text-sm p-2 border border-blue-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 bg-blue-50" value={editTxt} onChange={(e) => setEditTxt(e.target.value)} rows={2} />
              <div className="flex gap-2 mt-2 justify-end">
                <button onClick={() => setIsEditingC(false)} className="text-xs text-gray-500 font-bold px-2 py-1 hover:bg-gray-100 rounded">Batal</button>
                <button onClick={() => { handleEditComment(item, comment.id, editTxt); setIsEditingC(false); }} className="text-xs bg-blue-600 text-white font-bold px-3 py-1 rounded hover:bg-blue-700">Simpan</button>
              </div>
            </div>
          ) : (
            <>
              <p className="text-sm text-gray-700 mt-1 leading-relaxed break-words break-all whitespace-pre-wrap">{comment.text}</p>
              <button onClick={() => setReplyingTo({ itemId: item.id, commentId: comment.id })} className="text-[11px] text-red-500 font-bold mt-2 flex items-center gap-1 hover:underline">
        ...
        {isReplying && (
          <div className="mt-2 ml-2 p-3 bg-white rounded-xl border border-red-200 shadow-lg shadow-red-100/50 z-10 relative animate-in fade-in zoom-in-95">
            ...
            <div className="flex gap-2">
              <input type="text" autoFocus placeholder="Tulis balasan..." className="flex-1 text-xs p-2 border border-red-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 bg-gray-50" 
                value={commentInputs[item.id] || ''} onChange={(e) => setCommentInputs({ ...commentInputs, [item.id]: e.target.value })} 
                onKeyDown={(e) => e.key === 'Enter' && handleSendComment(item, comment.id)} 
              />
  ...
  return (
    <div className="min-h-screen bg-[#FFF0F0] p-6 md:p-12 font-sans text-gray-800 relative selection:bg-red-200">
              ...
              <div>
                ...
                <div className="flex gap-2 overflow-x-auto py-2">
                  {editAttachments.map((att) => (
                    <div key={att.id} className="relative w-20 h-20 flex-shrink-0 group bg-gray-100 rounded-lg flex items-center justify-center border border-gray-200">
                      {att.type === 'video' ? <Film className="text-gray-400" /> : att.type === 'file' ? <div className="text-center p-1"><FileText className="text-gray-400 mx-auto" size={20} /><span className="text-[8px] block truncate w-16">{att.fileName}</span></div> : <img src={att.url} className="w-full h-full object-cover rounded-lg" />}
                  ...
                  <label className="w-20 h-20 border-2 border-dashed border-red-200 rounded-lg flex flex-col items-center justify-center cursor-pointer hover:bg-red-50">
                    ...
                    <input type="file" multiple onChange={(e) => handleFileUpload(e, true)} disabled={isUploadingEdit} className="hidden" />
                  </label>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 text-center relative">
            ...
            <div className="flex gap-3">
              <button onClick={() => setItemToDelete(null)} className="flex-1 py-3 text-gray-600 bg-gray-100 rounded-xl font-bold">Batal</button>
              <button onClick={confirmDelete} disabled={deleteConfirmationText !== itemToDelete.title} className="flex-1 py-3 bg-[#B91C1C] text-white rounded-xl font-bold hover:bg-red-800 disabled:opacity-50">Hapus</button>
            </div>
          </div>
        </div> 
      )}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* INPUT FORM (HANYA MUNCUL SAAT TIDAK SEARCH) */}
        {!searchQuery && (
            ...
            <form onSubmit={handleAddItem} className="space-y-5">
                <div className="flex flex-col md:flex-row gap-5">
                  <div className="flex-1 space-y-2">
                    ...
                    <input type="text" placeholder="Contoh: Motion Graphic 2025" className="w-full p-4 border-2 border-gray-100 bg-[#FAFAFA] rounded-2xl focus:outline-none focus:border-[#B91C1C] focus:bg-white transition-all font-bold text-gray-800 placeholder:font-normal" value={newItemTitle} onChange={(e) => setNewItemTitle(e.target.value)} required />
                  </div>
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex flex-col gap-3">
                      <label className={`cursor-pointer w-full p-4 border-2 border-dashed border-red-200 rounded-2xl flex items-center justify-center font-bold text-[#B91C1C] transition-colors ${isUploading ? 'bg-red-50 opacity-50' : 'hover:bg-[#FFF0F0] hover:border-[#B91C1C]'}`}>
                          ...
                          {isUploading ? 'Mengupload...' : 'Pilih File (Gambar/Video/Rar)'}
                      ...
                      {newItemAttachments.length > 0 && (
                        <div className="flex gap-2 overflow-x-auto pb-2">
                          {newItemAttachments.map(att => (
                            <div key={att.id} className="relative w-16 h-16 flex-shrink-0 bg-gray-100 rounded-lg flex items-center justify-center border border-gray-200">
                              {att.type === 'video' ? <Film className="text-gray-500" size={20}/> : att.type === 'file' ? <FileText className="text-gray-500" size={20}/> : <img src={att.url} className="w-full h-full object-cover rounded-lg" />}
            ...
            <div key={item.id} className={`bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col transition-all duration-300 ${item.isDone ? 'opacity-60 grayscale-[0.8]' : 'hover:shadow-2xl hover:shadow-red-100/50 hover:-translate-y-2'}`}>
              <div className="rounded-t-3xl overflow-hidden relative">
                 ...
                 <div className="absolute top-4 right-4 flex gap-2 z-20">
                   <button onClick={() => startEdit(item)} className="bg-white text-gray-700 hover:text-[#B91C1C] p-3 rounded-full shadow-lg font-bold transition-all"><Pencil size={18} /></button>
                   <button onClick={() => setItemToDelete(item)} className="bg-white text-gray-700 hover:text-red-600 p-3 rounded-full shadow-lg font-bold transition-all"><Trash2 size={18} /></button>
                 </div>
              </div>
