...
export default function InspirationBoard() {
  ...
  // ----------------------------------------------------------------
  const uploadToCloudinary = (file: File, onProgress: (loaded: number, total: number) => void): Promise<{ success: boolean, url?: string, error?: string }> => {
    return new Promise((resolve) => {
      const url = 'https://api.cloudinary.com/v1_1/dl2ijoilh/auto/upload';
  ...
  const handleFileInput = async (e: React.ChangeEvent<HTMLInputElement>, isEditMode: boolean = false) => {
    const files = e.target.files;
    if (files && files.length > 0) {
      await processFiles(files, isEditMode);
    }
  };

  const processFiles = async (fileList: FileList, isEditMode: boolean) => {
    if (isEditMode) setIsUploadingEdit(true); else setIsUploading(true);
    setUploadProgress(0);

    const files = Array.from(fileList);
    const totalSizes = files.map(f => f.size);
    ...
    const uploadPromises = files.map((file, index) =>
      uploadToCloudinary(file, (loaded) => updateProgress(index, loaded))
        .then(result => {
          if (!result.success) {
            return { error: result.error, fileName: file.name, isError: true };
          }
          let type: 'image' | 'video' | 'file' = 'file';
          if (file.type.startsWith('image/')) type = 'image';
          else if (file.type.startsWith('video/')) type = 'video';
          return { id: Date.now() + Math.random().toString(), type, url: result.url!, fileName: file.name, isError: false };
        })
    );
    ...
    results.forEach(res => {
      if (res.isError) {
        failedUploads.push({ fileName: (res as any).fileName, error: (res as any).error });
      } else {
        const { isError, ...attachment } = res as any;
        successfulUploads.push(attachment);
      }
    });

    if (failedUploads.length > 0) {
      const errorMsg = failedUploads.map(f => `- ${f.fileName}: ${f.error}`).join('\n');
      alert(`Beberapa file gagal diupload:\n${errorMsg}`);
    }

    if (successfulUploads.length > 0) {
      if (isEditMode) {
        setEditAttachments(prev => sortAttachments([...prev, ...successfulUploads]));
      } else {
        setNewItemAttachments(prev => sortAttachments([...prev, ...successfulUploads]));
      }
    }

    if (isEditMode) setIsUploadingEdit(false); else setIsUploading(false);
    setUploadProgress(0);
  };
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex flex-col gap-3">
                      <label
                        ...
                        onDrop={handleDrop}
                        className={`cursor-pointer w-full p-6 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center font-bold transition-all duration-300 ${isDragging ? 'bg-red-100 border-[#B91C1C] scale-105 shadow-xl' : isUploading ? 'bg-gray-50 border-gray-200 opacity-60' : 'bg-white border-red-200 hover:bg-[#FFF0F0] hover:border-[#B91C1C]'}`}
                      >
                        {isUploading ? (
                          <div className="w-full max-w-[200px] text-center">
                            <div className="flex items-center justify-center gap-2 mb-2 text-[#B91C1C]">
                              <Loader2 className="animate-spin" />
                              <span>Mengupload {uploadProgress}%</span>
                            </div>
                            <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                              <div className="h-full bg-[#B91C1C] transition-all duration-300" style={{ width: `${uploadProgress}%` }}></div>
                            </div>
                          </div>
                        ) : (
                          <>
                            <UploadCloud className={`mb-2 ${isDragging ? 'text-[#B91C1C] scale-110' : 'text-gray-400'}`} size={32} />
                            <span className="text-[#B91C1C]">
                              {isDragging ? 'Lepaskan File Disini' : 'Klik atau Drag & Drop File'}
                            </span>
                            <span className="text-[10px] text-gray-400 font-normal mt-1">Gambar, Video, atau Dokumen</span>
                          </>
                        )}
                        <input type="file" multiple onChange={(e) => handleFileInput(e, false)} disabled={isUploading} className="hidden" />
                      </label>
