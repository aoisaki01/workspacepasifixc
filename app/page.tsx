...
import { Trash2, CheckCircle, MessageSquare, Plus, Image as ImageIcon, Loader2, UploadCloud, X, AlertTriangle, Pencil, Save, CornerDownRight, ChevronDown, ChevronUp, Play, Film, FileImage, FileText, Download, Paperclip } from 'lucide-react';
...
interface Attachment {
  id: string;
  type: 'image' | 'video' | 'file'; // ✅ Tambah tipe 'file'
  url: string;
  fileName?: string; // ✅ Simpan nama file asli
}
...
export default function InspirationBoard() {
  ...
  // ----------------------------------------------------------------
  // 2. HELPER & SORTING (Priority: Image -> Video -> File)
  // ----------------------------------------------------------------
  ...
  // ----------------------------------------------------------------
  // 3. FUNGSI UPLOAD (ALL FILES SUPPORT)
  // ----------------------------------------------------------------
  const uploadToCloudinary = async (file: File): Promise<string | null> => {
    ...
    try {
      // Gunakan 'auto' agar Cloudinary menerima PDF/ZIP sebagai 'raw' atau 'image'
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (url) {
        ...
        newUploaded.push({
          ...
          type,
          url,
          fileName: file.name // Simpan nama asli
        });
      }
    }
  ...
  // ----------------------------------------------------------------
  // 6. UI COMPONENT: MEDIA PREVIEW (UPDATE: SUPPORT FILE)
  // ----------------------------------------------------------------
          ...
          {attachments.map((att, index) => (
            <div key={index} className="w-full flex-shrink-0 snap-center h-full relative flex items-center justify-center bg-gray-100">
              ...
              {att.type === 'video' ? (
                ...
                // TAMPILAN KHUSUS FILE (PDF/ZIP/DOC)
                <div className="flex flex-col items-center justify-center p-6 text-center h-full w-full bg-slate-100 text-slate-600">
                  ...
                  <span className="text-sm font-bold text-gray-800 line-clamp-1 max-w-[80%] mb-4 px-2">
                    {att.fileName || 'Dokumen Terlampir'}
                  </span>
                  <a 
                    href={att.url} 
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 bg-[#B91C1C] text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-red-800 transition-colors"
                  >
                  ...
                  {editAttachments.map((att) => (
                    <div key={att.id} className="relative w-20 h-20 flex-shrink-0 group bg-gray-100 rounded-lg flex items-center justify-center border border-gray-200">
                      {att.type === 'video' ? (
                         ...
                         <div className="text-center p-1"><FileText className="text-gray-400 mx-auto" size={20} /><span className="text-[8px] block truncate w-16">{att.fileName}</span></div>
                      ) : (
            ...
            <form onSubmit={handleAddItem} className="space-y-5">
                <div className="flex flex-col md:flex-row gap-5">
                  ...
                  <div className="flex-1 space-y-2">
                    <label className="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">Lampiran (Foto/Video/Dokumen)</label>
